...
When I initially pulled all the data[^1], I thought I had made a mistake. I was expecting every shot Tim Duncan took in the 2015-16 season, but I only had `r nrow(game)` shots. Well, that number turned out to be correct. Tim played `r length(unique(game$GAME_ID))` regular season games, and averaged `r round(nrow(game)/length(unique(game$GAME_ID)), 1)` FGA/game, for a total of `r nrow(game)` field goal attempts. (Fun fact: I omitted Duncan's player ID the first time I ran this and actually pulled back all FGA taken in the season: there were 207,895. James Harden had the most, with 1,617). Here's Duncan's shot chart over the entire regular season (note: 1 shot from behind half-court omitted).  
...
ggplot(game, aes(x = LOC_X * -1, y = LOC_Y * -1, z = as.numeric(SHOT_MADE_FLAG) - 1)) + 
  ...
  annotate("text", x = 190, y = -405, size = 6, fontface = 2,
           label = paste0(round(sum(game$SHOT_MADE_FLAG ==1)/nrow(game)*100, 2), "% Overall")) +
  stat_summary_hex() +
...
```{r}
duncan_shots <- nrow(game)
duncan_shots_RA <- nrow(game[game$SHOT_ZONE_BASIC == "Restricted Area", ])
duncan_makes_RA <- nrow(game[game$SHOT_ZONE_BASIC == "Restricted Area" & 
                               game$SHOT_MADE_FLAG == 1, ])
duncan_shots_non_RA <- nrow(game[game$SHOT_ZONE_BASIC == "In The Paint (Non-RA)", ])
duncan_makes_non_RA <- nrow(game[game$SHOT_ZONE_BASIC == "In The Paint (Non-RA)"
                                 & game$SHOT_MADE_FLAG == 1, ])
duncan_shots_8_16 <- nrow(game[game$SHOT_ZONE_RANGE == "8-16 ft." ,])
duncan_makes_8_16 <- nrow(game[game$SHOT_ZONE_RANGE == "8-16 ft." & game$SHOT_MADE_FLAG == '1',])
duncan_shots_16_24 <- nrow(game[game$SHOT_ZONE_RANGE == "16-24 ft." ,])
duncan_makes_16_24 <- nrow(game[game$SHOT_ZONE_RANGE == "16-24 ft." & game$SHOT_MADE_FLAG == '1',])
```

It's no surprise that Timmy's strongest areas are in the paint, especially within the restricted area, and especially to the left of the rim. `r round(duncan_shots_RA/duncan_shots * 100, 0)`% of Duncan's shots last season came from the restricted area, which he converted at a `r round(duncan_makes_RA / duncan_shots_RA * 100, 1)`% clip[^2]. From the rest of the painted area he cooled down a bit, shooting `r round(duncan_shots_non_RA/duncan_shots * 100, 0)`% of his shots from there, converting at a `r round(duncan_makes_non_RA/duncan_shots_non_RA * 100, 1)`% rate. Unsurprisingly, Tim's numbers drop as the shot attempts move further out; from 8-16 ft he converted `r round(duncan_makes_8_16 / duncan_shots_8_16 * 100, 1)`% and from 16-24 ft he converted `r round(duncan_makes_16_24 / duncan_shots_16_24 * 100, 1)`%. There were `r nrow(game[game$SHOT_TYPE== "3PT Field Goal",])` failed attempts from beyond the arc, but it's Tim Duncan, and he made it [when it counted](https://www.youtube.com/watch?v=trdsKp94Io0). 

His `r round(sum(game$SHOT_MADE_FLAG ==1)/nrow(game) * 100, 1)`% overall field goal percentage for the season was pretty good compared to the league average of 45.2%.

Adding in his 1.5 made free throws per game[^3], Duncan ended up contributing 8.6 PPG in the 2015-16 season, by far his lowest. While it's true that he also played the fewest minutes of his career (25.2 MPG), his individiual points per minute[^4] suffered a career low, finishing at 0.34 (his career high was 0.63 iPPM in 2001-02). While seeing Timmy retire might be the low point of my life, it was probably for the best. But he went out fighting, and I love that.  
...
Gasol had an overall FG% last season of `r round(sum(gasol$SHOT_MADE_FLAG ==1)/nrow(gasol) * 100, 1)`%, less than Tim's `r round(sum(game$SHOT_MADE_FLAG ==1)/nrow(game)*100, 1)`%. His iPPM of 0.52 is higher than Tim's, indicating that he was shooting more frequently. Here is Gasol's 2015-16 shot chart.  
...
LaMarcus's numbers were solid from all over the court last season (except from downtown); he averaged an overall `r round(aldridge_makes/aldridge_shots * 100, 1)` FG%. In the restricted area he shot a stellar `r round(aldridge_makes_RA / aldridge_shots_RA * 100, 1)`%. That's good news for the Spurs. They may be able to move Aldridge into more of a center position on offense, and keep Gasol further back, where he can hit those mid-to long-range shots and the occasional three.  
...
Those numbers are very close to what they were for both players last season, however, this gives the Spurs a front court that can shoot from almost anywhere on the floor. Aldridge is listed as 6'11", the same height as Tim, and 10 pounds leaner, at 240 lbs. He should be able to consistently play the position. Gasol has taken heat throughout his career of being soft, moving him to the 4 might help that. One thing I'm unsure about is if he has enough speed to play the position on offense.  

I'm hesitant to explore how this affects the Spurs' total points, since there's no telling what's going to happen with bench minutes. It's very possible that Gasol could get the same minutes he did last season with Chicago, 31.8, since that's about what Aldridge got. Regardless of how Popovic manages his team's minutes, it looks like if the Spurs stay healthy they'll be able to cover on offense what they lose with Duncan.  
