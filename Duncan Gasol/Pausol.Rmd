...
When I initially pulled the data[^1], I thought there had been made a mistake. I had every shot taken by every player last season, `r prettyNum(nrow(game), big.mark = ",")` attempts in all. James Harden led all players with `r prettyNum(nrow(game[game$PLAYER_NAME == "James Harden", ]), big.mark = ",")`. Steph Curry was next with`r prettyNum(nrow(game[game$PLAYER_NAME == "Stephen Curry", ]), big.mark = ",")`. But for Duncan, there were only `r nrow(duncan)` shots. That couldn’t be right. But it was. Tim played `r length(unique(duncan$GAME_ID))` regular season games, and averaged `r round(nrow(duncan)/length(unique(duncan$GAME_ID)), 1)` FGA/game. He indeed had just `r nrow(duncan)` shot attempts last season. Here’s Duncan’s shot chart over the entire regular season (note: 1 shot from behind half-court omitted).  
